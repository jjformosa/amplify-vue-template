...
const doSomething = async () => {
  ...
  // console.log(`response.length: ${responseIds.length}`)
  const questionsOfTopic: string[] = []
  const topic1 = topicResult[0]
  let index = 0
  for (const scenarioData of scenarios1) {
    ...
    if (question) {
      ...
      const scenario = (await clientAssessmentTopic.models.Scenario.create({
  ...
  const topic2 = topicResult[1]
  index = 0
  for (const scenarioData of scenarios2) {
    const question = (await clientAssessmentTopic.models.Question.create({
      topicId: topic2.id,
      response: responseIds
    })).data
  ...
  const topic3 = topicResult[2]
  index = 0
  for (const scenarioData of scenarios3) {
    const question = (await clientAssessmentTopic.models.Question.create({
      topicId: topic3.id,
      response: responseIds
    })).data
  ...
  const topic4 = topicResult[3]
  index = 0
  for (const scenarioData of scenarios4) {
    const question = (await clientAssessmentTopic.models.Question.create({
      topicId: topic4.id,
      response: responseIds
    })).data
