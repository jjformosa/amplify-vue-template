...
const doSomething = async () => {
  ...
  let scenariosQuery = (await clientAssessmentTopic.models.Scenario.list({
    filter: {
      i18n: { eq: 'zh-tw' }
    }
  }))
  console.log(`length: ${scenariosQuery.data.length}`)
  let nextToken = scenariosQuery?.nextToken
  do {
    for (const scenario of scenariosQuery.data) {
      clientAssessmentTopic.models.Scenario.update({
        id: scenario.id,
        i18n: 'zh_tw'
      })
    }
    scenariosQuery = (await clientAssessmentTopic.models.Scenario.list({
      filter: {
        i18n: { eq: 'zh-tw' }
      },
      nextToken
    }))
    nextToken = scenariosQuery.nextToken
  }
  while(scenariosQuery.data)
}
